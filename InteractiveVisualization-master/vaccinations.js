...
x = d3v6.scaleTime().range([0, width])
y = d3v6.scaleLinear().range([height, 0])

allCountries = ['United States', 'Canada', 'India', 'Brazil', 'United Kingdom', 'China', 'Italy']

xAxisCall = d3v6.axisBottom().tickFormat(d3v6.timeFormat("%b' %y"))
xAxis = g.append('g')
  .attr('transform', `translate(0, ${height})`)
yAxisCall = d3v6.axisLeft()
yAxis = g.append('g')

parser = d3v6.timeParse(d3v6.timeParse('%Y-%m-%d'))

let sorted

categoryScale = d3v6.scaleOrdinal().domain(['single-dose', 'fully-vaccinated', 'fully-vaccinated-ph'])
  .range(['people_vaccinated', 'people_fully_vaccinated', 'people_fully_vaccinated_per_hundred'])

colorScale = d3v6.scaleOrdinal().domain([allCountries]).range(['blue', 'red', 'orange', 'green', 'violet', 'darkred', 'lightgreen'])

symbolScale = d3v6.scaleOrdinal().domain(['people_vaccinated', 'people_fully_vaccinated', 'people_fully_vaccinated_per_hundred']).range(['People Vaccinated', 'People Fully Vaccinated', '% Fully Vaccinated'])
...
function render(data, symbol) {
  ...
  let tooltip = d3v6.select('#tooltip').style('position', 'relative').style('left', '30px')
      ...
      .on('mouseover', function () {
        ...
        tooltip.transition()
          ...
          .style('background-color', () => {
            let color = d3v6.rgb(colorScale(data[i].key))
            color.opacity = 0.3
            return color
          })
      })
